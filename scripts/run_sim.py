...
def get_cache_configs(peak_mem, num_cores, simulator, exp):
    configs = []
...
def get_dram_configs(peak_mem, num_cores):
    ...
    config_name = "dram"
    return {"DRAM"          : "{},{},{},{}".format(page_size, total_size, local_size, remote_size),
                    "dram_config"  : config_name}
    ...
    for a, details in apps.items():
        ...
        for r in runs:
            for m in details["methods_valid"]:
                ...
                base = get_cache_configs(peak_mems[m], num_cores[m], simulator, exp)
                dram_config = get_dram_configs(peak_mems[m], num_cores[m])
